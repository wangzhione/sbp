...
这里 `cast` 是对 `[spf13/cast](https://github.com/spf13/cast)` 补充, 二者可以结合使用.

`spf13/cast` 库非常出名, 特别是 PHP 类似运行时语言转 Golang 开发很喜欢. 

在 Golang 开发中, 遇到通信 RPC 双方跨语言协议也适合尝试, 用起来爽感很足. 

其实大部分业务转换无外乎 int 和 string 直接交互, 或者 time 业务.

常见基本转换都能从 cast.go 找到解法
